...
def load_weights(model_path:str, device=None):
  index = f"{model_path}/model.safetensors.index.json"

  with open(index) as fp:
    weight_map = json.load(fp)["weight_map"]
    files = set(weight_map.values())

    parts = {fn:safe_load(f"{model_path}/{fn}") for fn in files}
    weights = {k:parts[v][k].to(device) for k,v in weight_map.items()}

    return weights

class LLaMa:
  @staticmethod
  def build(model_path, tokenizer_path, model_size="7B", device=None):
    ...
    with WallTimeEvent(BenchEvent.LOAD_WEIGHTS):
      if model_path.is_dir():
        weights = load_weights(model_path, device[0] if isinstance(device, tuple) else device)
      else:
...
if __name__ == "__main__":
  ...
  llama = LLaMa.build(MODEL_PATH, TOKENIZER_PATH, model_size=args.size, device=device)
